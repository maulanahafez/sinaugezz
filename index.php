<?php
include("./inc/header.php");
?>
<div class="banner-bg position-relative" style="background-image: url('./img/9-SCENE.png'); ">
  <div class="container pb-5 pt-md-4">
    <div class="row py-5 justify-content-between align-items-center">
      <div class="col-lg-6 col-md-7 py-5">
        <h1 class="font-weight-bold display-4 mb-4">Our tutors offers face-to-face and online tuition. </h1>
        <p class="lead mb-3">Our focus - modular training programs from leading practice lectors.</p>
        <a href="" class="btn btn-success">Sign in &rarr; </a>
      </div>
    </div>
  </div>
</div>
</div>
<?php
include("./inc/footer.php");
?>